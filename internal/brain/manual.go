...
• *옵시디언에서 쓰던 .md를 그냥 첨부하면* → 같은 주제 노트가 있는지 찾아보고 어디에 둘지·status를 추천해요. 파일 맨 위에 적어두신 프론트매터는 그대로 살리고 빈 칸만 채웁니다. 이게 제일 편한 길이에요.
• *공부한 글을 그냥 붙여넣으면* → 똑같이 추천해요. 이쪽도 그대로 됩니다.
• *어느 쪽이든 "만들어라"를 같이 적으면* → 추천한 자리로 PR을 엽니다. 머지는 직접 하시면 돼요.
• *"X 정리한 거 있어?"* → 위키에서 찾아 경로랑 링크로 답해요.
• *"위키 현황"* → 노트 수랑 status 분포.

승인 낱말: 만들어라 / 올려줘 / 넣어줘 / PR / ㄱㄱ / 그래 / 동의
저는 앞 메시지를 기억하지 못해요 — 승인은 *초안과 같은 메시지에* 넣어주세요. 파일도 그때 다시 첨부해주시면 됩니다.
붙여넣기로 보내실 땐 본문을 마크다운 코드블록에 넣어주세요. 슬랙 입력창이 마크다운을 먹어서, 그냥 붙이면 "- "는 서식 불릿이 되고 "#"은 사라져요. 블록 자국은 제가 걷어내고 넣습니다. 파일로 주시면 이 걱정은 없어요.
본문은 어느 쪽이든 보내주신 글자 그대로 들어가요. 제가 고쳐 쓰지 않습니다.`
